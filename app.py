...
def index(machineTyp, machineNo):
    ...
    # 搜尋資料庫是否有此機型或機號，limit就是只要有就停止搜尋，代表是有項目的
    sql = 'select machineTyp,machineNo from user where machineTyp="' + \
        machineTyp + '"' + 'and machineNo="'+machineNo+'" limit 1'
    # 將搜尋到的資料存放在datas下備用，給index.html提取資料用
    datas = db.query_data(sql)
